...
Duomenys įkelti į "Egzaminas 2016-06-15" aplanką GitHub'e. Nuskaitomi duomenys:
```{r}
duomvisi <- read.csv2("data_a.csv", header = TRUE)
duomvisi$amzius[duomvisi$amzius=="MISSING"] <- NA
complete.cases(duomvisi)
x <- na.omit(duomvisi)
duomvisi <- read.csv2("data_a.csv", header = TRUE)
duomvisi[duomvisi=="MISSING"] <- NA
complete.cases(duomvisi)
x <- na.omit(duomvisi)
```
